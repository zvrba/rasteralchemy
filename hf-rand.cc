...
static char rcsid[] UNUSED = "$Id: hf-rand.cc,v 1.1.2.3 2003/12/31 15:25:18 zvrba Exp $";
...
*/
	...
	if (ij<0 || ij>31328 || kl<0 || kl>30081) {
		fputs("ERROR: rmarin: The first random number seed must have a value between 0 and 31328.", stderr);
		fputs("The second seed must have a value between 0 and 30081.", stderr);
		exit(1);
	}
...
/*  the output is a number between 0.0 and 1.0 */
{
	...
	if (test==FALSE) {
	   fprintf(stderr, "ERROR: ran1: Call the init routine seed_ran1() before calling ran1().");
		exit(1);
	}
